...
describe('GET /api/tasks/:taskId/comments', () => {
  ...
  it('should return empty data with correct meta structure when no comments exist', async () => {
  ...
  it('should return 400 for invalid pagination params', async () => {
    try {
      await axios.get(withUrl(`/api/tasks/${task.id}/comments?page=0`));
      fail('Expected request to fail');
    } catch (error: any) {
  ...
  it('should only return comments for the specified task', async () => {
    ...
    expect(res.data.data[0].text).toBe('My Comment');
  });
});
