...
describe('GET /api/workspaces/:id/chats', () => {
  ...
  describe('with no chats', () => {
    ...
    it('should respect custom page parameter', async () => {
      const res = await axios.get(
        withUrl(`/api/workspaces/${workspace.id}/chats?page=2`),
      );
    ...
    it('should respect custom limit parameter', async () => {
      const res = await axios.get(
        withUrl(`/api/workspaces/${workspace.id}/chats?limit=5`),
      );
  ...
  describe('validation', () => {
    it('should return 404 for non-existent workspace', async () => {
      try {
        await axios.get(withUrl('/api/workspaces/nonexistent/chats'));
    ...
    it('should return 400 for page less than 1', async () => {
      try {
        await axios.get(
          withUrl(`/api/workspaces/${workspace.id}/chats?page=0`),
        );
        fail('Expected request to fail');
      } catch (error: any) {
    ...
    it('should return 400 for limit greater than 100', async () => {
      try {
        await axios.get(
          withUrl(`/api/workspaces/${workspace.id}/chats?limit=101`),
        );
        fail('Expected request to fail');
      } catch (error: any) {
        expect(error.response.status).toBe(400);
  ...
  describe('with chats', () => {
    ...
    it('should paginate correctly', async () => {
      ...
      const res = await axios.get(
        withUrl(`/api/workspaces/${workspace.id}/chats?limit=2`),
      );
